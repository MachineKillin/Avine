...
{'User-Agent' : 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.93 Safari/537.36'}] #wow even more headers
Results = [] 
GoodList = [] 
Total_Found = 0 
Valid = 0 
...
def parser2():
    ...
    Drok()
    proxy()
    threading.Thread(target=parse, args=()).start() #i would just make this a for loop for people who want custom threads maybe in an update in 2050 :D
    threading.Thread(target=parse, args=()).start() 
    threading.Thread(target=parse, args=()).start()
    threading.Thread(target=parse, args=()).start()
    threading.Thread(target=parse, args=()).start()
